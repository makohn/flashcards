...
public class BarChart2D extends JPanel {
	...
	private static final int BAR_WIDTH_THIN  = 25;
	private static final int BAR_WIDTH_THICK = 25;
	
	private static final int BAR_WIDTH_TOTAL = BAR_WIDTH_THICK + BAR_WIDTH_THIN;
	...
	private int size_y = 200; //TODO: variabel gestalten
	private int size_x;
	private int legend_height;
	private int legend_width;
	
	public BarChart2D(int[] values1, int[] values2) {
		super.setOpaque(false);
		this.values1 = values1;
		this.values2 = values2;
		size_x = values1.length * (BAR_WIDTH_TOTAL+GAP) + GAP;
	}
...
//		    	g2.drawLine(ORIGIN_X, i, ORIGIN_X+size_x, i);
		    ...
		    step = BAR_WIDTH_TOTAL + GAP;
		    int y = ORIGIN_Y+size_y;
		    j = 1;
		    for(int i = ORIGIN_X+step-BAR_WIDTH_TOTAL/2; i<=size_x; i=i+step) {
		    	g2.setColor(Color.black);
		    ...
		    for(int i = 0; i<values1.length;i++ ) {
		    	...
		    	int start_X = ORIGIN_X+GAP+i*(BAR_WIDTH_TOTAL+GAP);
		    	int start_Y = ORIGIN_Y+size_y-height-1;
		   
		    	GradientPaint gp = new GradientPaint(start_X, start_Y, COLOR_BLUE.brighter(), 
		    			start_X+BAR_WIDTH_THICK, start_Y+height, COLOR_BLUE);
		    	g2.setPaint(gp);
		    	g2.fillRect(start_X, start_Y , BAR_WIDTH_THICK, height);
		    	g2.setColor(COLOR_BLUE.darker());
		    	g2.drawRect(start_X, start_Y , BAR_WIDTH_THICK, height);
		    ...
		    for(int i = 0; i<values2.length;i++ ) {	
		    	...
		    	int start_X = ORIGIN_X+GAP+i*(BAR_WIDTH_TOTAL+GAP);
		    	int start_Y = ORIGIN_Y+size_y-height-1;
		    	GradientPaint gp = new GradientPaint(start_X+BAR_WIDTH_THICK, start_Y, COLOR_RED.brighter(), 
		    			start_X+BAR_WIDTH_THIN, start_Y+height, COLOR_RED);
		    	g2.setPaint(gp);
		    	g2.fillRect(start_X+BAR_WIDTH_THICK, start_Y , BAR_WIDTH_THIN, height);
		    	g2.setColor(COLOR_RED.darker());
		    	g2.drawRect(start_X+BAR_WIDTH_THICK, start_Y , BAR_WIDTH_THIN, height);
		    	g2.drawString(values2[i]+"", start_X+BAR_WIDTH_THICK, start_Y-5);
		    } 
